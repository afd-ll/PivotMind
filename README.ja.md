...
**現行バージョン: v0.4.7** — 13 の脳領域/サブシステム（全実装済み）、創発的品詞システム、並列マルチ学習器、PFE 推論、512 次元特徴ベクトル。
...
**v0.4.7 改善**: 機能語フィルタリング — `is_function_word()` が~130 語の中英機能語をチェックし、パイプラインの3段階（アクティブセット更新、重み付きスコアリング、出力）でフィルタリングして、高接続性の機能語が出力を支配するのを防止します。側方抑制により内容語の多様性を確保します。
...
| **v0.4.7**  | 拡散エンジン機能語フィルタ(~130語)、クロスレイヤーインデックス修正、double-free 修正  |
