...
**現行バージョン: v0.5.9** — 14 の脳領域（全実装済み）、創発的品詞システム、並列マルチ学習器、PFE 推論、512 次元特徴ベクトル、**語彙層セマンティックフィールド**、**話題性生成**、**PFE 推論パイプライン**、**非同期知覚探索**、**統計表メモリ化（Hebbian 強化、短期/長期階層）** — 14 の脳領域/サブシステム（全実装済み）、創発的品詞システム、並列マルチ学習器、PFE 推論、512 次元特徴ベクトル、**語彙層セマンティックフィールド（語彙強化 + セマンティックトポロジー分類）**、**話題性生成（関連性スコアリング + 有界連想）**、**PFE 推論パイプライン（サブゴール分解 + 因果探索 + 推論チェーン）**。
...
PivotMind は哺乳類の皮質機能区分をモデル化します — 14 の脳領域/サブシステムがそれぞれ専任の責務を持ち、Thalamus 信号バスを介して通信します。**全13領域が完全実装済みで、スタブコードはゼロです。**
...
**v0.5.5 改善**: 機能語フィルタリング — `is_function_word()` が~130 語の中英機能語をチェックし、パイプラインの3段階（アクティブセット更新、重み付きスコアリング、出力）でフィルタリングして、高接続性の機能語が出力を支配するのを防止します。側方抑制により内容語の多様性を確保します。
...
├── changelogs/        # 58 バージョン変更履歴(000-057)
...
| **v0.5.5**  | 拡散エンジン機能語フィルタ(~130語)、クロスレイヤーインデックス修正、double-free 修正  |
| **v0.5.6** | rwlock デッドロック修正（書き込みロック内のネスト読み取り）、gateway トポロジ容量、知識生存保護（ロード保護 + フロア活性化） |
| **v0.5.9** | **統計表メモリ化**（Hebbian 強化、短期/長期階層、降権削除なし）、**非同期知覚探索**（ワーカーキュー）、**/learn キュー化**（スレッド 35→6）、**POS プールパイプライン** |
| **v0.5.7** | **語彙層セマンティックフィールド**（語彙強化、語-語共起エッジ、セマンティックトポロジー分類、セマンティックフィールド問い合わせ）、**話題性生成**、**PFE 推論パイプライン**、知識生存全面強化 |
