...
> [changelogs/094-v069-sg-threshold.md](changelogs/094-v069-sg-threshold.md)。

### 核心变更（1 处真实行为变化）

- **BG-24 — `SG_COSINE_THRESHOLD` 0.02f → 0.8748f**：该阈值的注释口径是「256 维**随机向量**几乎正交」，
  但 `semantic_grow_from_topology()` 读的特征是**已被拉普拉斯平滑**的向量
  （平滑 3 步后两两余弦 p10 = 0.318 / p50 = 0.566 / **p99 = 0.875**）
  ⇒ 旧值落在分布之外，**通过率 99.92%** ⇒ 阈值不筛任何东西，聚类成员退化为
  「先到先得 + 偶然配对」（线上实证：814 个簇里 **63.9% 是纯 2 元簇**，最大 74 元簇是最早建的那批）。
- **新增聚类日志**：`[语义生长] <拓扑> 阈值=… 采样=… 有效=… 已归簇跳过=… 建簇=… 各簇大小=[…]`
  —— 没有它就无法判断阈值改得对不对（本次就是因为没有，只能靠解析 `sem_<x>_<N>` 反推簇大小）。
- **运行期回退开关** `PIVOTMIND_SG_THRESHOLD_LEGACY=1` ⇒ 用回 0.02（对比实验 / 紧急回退）。

### Verified

- WSL 全量 `make -j20 all`：**0 告警**。
- 端到端 A/B（**同一二进制 + 回退开关**，空沙箱各喂 3 轮语料 + 等 75 s）：
  | 组 | 语义簇数 | 平均簇 | 最大簇 |
  |---|---|---|---|
  | legacy（0.02） | 7 | 10.57 | 36 |
  | **new（0.8748）** | **1** | 11.00 | 11 |
  ⇒ **新阈值下语义生长显著变慢**（75 s 窗口：7 → 1）。**这是预期收益**（少建的是"偶然配对"的噪声簇），
    且机制仍工作（建出 11 元簇）。⚠️ 窗口太短、样本小，**上线后需看日志一周再评估**。
- **状态格式不变（仍 v11）** ⇒ 无需迁状态、回退只需换二进制或设回退开关。

## v0.6.8 — 2026-09-17

> 来源：工作台 **BG-20**（老大拍板「按你的想法来」= 方案 D：出口闸）+ **BG-14**（入口端一并补）。
> 完整说明见 [changelogs/093-v068-bg20-output-gate.md](changelogs/093-v068-bg20-output-gate.md)。

### 核心变更（2 处真实行为变化）

- **BG-20 — 存量污染「出口闸」**：`concept_is_outputtable()`（`src/dialog_generate.c`）新增
  `bg20_is_polluted()`，三条判据：① 含 `_` **且含非 ASCII** ② 含 `→` 或「置信度」③ 含 `U+FE10–FE6F`。
  **不删数据、不动状态格式** ⇒ 影响半径 = 本函数（被 **20+ 出口**调用，含 `diffusion.c:978` 的走边选词）。
  改前实测：线上 5089 个有名节点里 **545 个（10.71%）** 命中污染判据。
- **BG-14 — `is_punctuation()` 的 3 字节分支补码位**：原只覆盖 `U+3000–303F` / `U+FF00–FFEF`，
  漏 CJK 竖排 / 兼容形式 / **小写变体标点** 三段 ⇒ 线上出现污染词 `馴﹔`。现补 `U+FE10–FE6F` 连续一段。

### Verified

- WSL 全量 `make -j20 all`：141 gcc 步、**0 告警**；`check-tools` 19/19。
- **全量 `make test` 32 通过 / 0 失败**；**`make asan-test` rc=0**。
...
第一版判据① 是「含 `_` 即拒」，`make test` / `asan-test` **双双 FAIL `test_concept_output`** ——
该单测锁死契约「`a_b` 应放行（`_` 合法且非行首 `sem_`）」，注释明确写「别把它改成宽松一点」。
⇒ **不放宽单测**，改为把判据做精细（加「含非 ASCII」前置）；复核后**拦截能力零损失**（416 / 5 不变）。

### 已知遗留（不在本版）

- **92 个 ASCII 碎片仍会输出**（`IG`/`GPI`/`CID`/`RTO`/`TCI`/`ERC`/`Wa`…），
  但其中混着真词（`AT`/`ID`/`IN`/`ON`/`BED`/`ER`/`AB`/`EQU`/`STO`…）⇒ **不能一刀切**，需单独立项。
- vocab 里的脏节点**仍在**（本版有意不删数据 —— 删节点要动状态与边结构，风险更大）。

## v0.6.7 — 2026-09-17

> 来源：工作台 BG-19（断回喂环）。完整说明见 [changelogs/092-v067-bg19-strip-scaffold.md](changelogs/092-v067-bg19-strip-scaffold.md)。

### 核心变更

- **喂料前剥离生成端「格式骨架」**：`/chat` 原先把 **AI 回复整段**喂进学习路径（两处），
  而回复含 `为了回答这个问题，我分步进行了思考：` / `N. A → B (置信度: NN%)` / `综合以上分析：`
  等排版构件 ⇒ 被当词建进 vocab。新增 `bg19_strip_scaffold()` 剥 4 类构件、**保留语义文字**。
- ⚠️ 只改「喂给学习的副本」，**回复本身不受影响**；分配失败回退原文。

### 与 BG-13 正交

BG-13（v0.6.6）修的是「`_` 不被当标点」这个入口；本版修更上游的「不该把骨架喂回去」。
v0.6.6 上线后增量已止住（抽查带 `_` = 0），但骨架词仍被建 ⇒ 正需本版。

### Verified

- WSL 全量 `make CC=gcc all`：141 gcc 步、0 告警；**全量 `make test` 32 通过 / 0 失败**。
...
- **状态格式 v11**：每节点记录尾部追加 4 字节 `float heat`（节点热度）。
  heat 此前**不落盘** ⇒ 每次加载全节点回创建默认 `1.0`，会话内冷却信号（走边单向衰减，floor 0.05）不跨会话。
- **写端两处同布局**：流式批 `master_serialize_batch` + 锁内回退路径；`SaveNodeSnap` 加字段并深拷贝。
- **读端**：Pass 1 `fmt_ver>=11` 读该值并夹取到 `[0.05, 1.0]`（NaN/越界回落 1.0）；Pass 2 同步 SKIP 该段。
  `fmt_ver<=10`（v2..v10）无此段 ⇒ 保持创建默认 1.0，**零迁移**。

### 行为变化（如实登记）

- 加载后不再「全员满热」：长期不活跃节点带 floor 0.05 进入下一会话（打分 `score *= 0.05 + 0.95*heat` ≈ ×0.0975）。
- 与 VF-02（`confidence` 恒 0.5）**不同根因**：confidence 是派生量、压根没接线；heat 是真实运行变量的跨会话化。
- ⚠️ **回退不可逆**：一旦用 v11 存盘，旧二进制（认 ≤10）会被格式闸门**显式拒绝**，换版前必须备份状态文件。

### Verified

- armbian-1（gcc 13.3.0）+ WSL（gcc 15.2.0）全量 `make CC=gcc all`：141 gcc 步、0 告警、`check-tools` 19/19。
- `make test-fast` **16 通过 / 0 失败**。
- 沙箱往返（线上状态只读副本，fmt_ver=10）：旧二进制读 v10（4383 节点）→ 新二进制读 v10（4433）→ 存盘写 **11** →
  新二进制读回 v11（4453）→ 再存盘仍 **11**；`fmt_ver=99` 被闸门拒绝。
...
### 修复 —— 锚定集回到「输入词 + 其组成字」（本版唯一真行为修复）

**病灶**：输入锚定有两条来源——**段A**（概念拓扑命中 → `cross_link` 桥 → 词汇层）与 **段B**（词汇拓扑直接命中），且两者共用同一个 break 判据 `active_count > 0`。实测：输入「悟空師父」激活的是 `閑/訣/是/舌`，**与输入零共享**。

1. **段A 默认关**（`PIVOTMIND_WALK_ANCHOR_STAGE_A=1` 可回退打开）。依据（同一快照 A/B）：
   开 ⇒ 有边 49.6% / 孤立 45.9%（锚定集含噪声节点）；关 ⇒ 有边 46.7% / 孤立 45.9%（锚定集 **100% 干净**）。
   ⇒ 净效果 ≈ 零，仅「有边 ↔ 共邻」内部重分配 ⇒ **干净起点优先**。
2. **段B 不再被短路**（`PIVOTMIND_WALK_ANCHOR_GUARD`，**默认开**）：原来段A 一旦有产出就 `break`，而段B 自己的 break 也用同一个
   被污染的 `active_count` ⇒ **段B 从未执行**。修后段B 全展开。依据：关 ⇒ 有边 34.1% / 开 ⇒ **49.6%**（**+15.5pt**）。

**效果**：锚定集从「不是输入词」变回「**输入词 + 其组成字**」，且空输出从 1/45 → **0/45**。

### 加入的调控开关（均默认 = 上表基线，全部可回退）

`PIVOTMIND_WALK_ALPHA`(0.85) · `_2HOP`(0.5) · `_REL`(1) · `_REL_DECAY`(0.7) · `_REL_BASE`(0.3) · `_REL_GAIN`(1.7)
· `_ANCHOR`(诊断) · **`_ANCHOR_GUARD`(默认开)** · **`_ANCHOR_STAGE_A`(默认关)** · `WALK_DUMP`(诊断)。

### Verified

- **armbian-1（aarch64 / gcc 13.3.0）**：`make clean && make all` **141 编译步 / 0 error / 0 warning**、`make tools` **19/19**、`make check-version` **PASS**、`make test` **32 通过 / 0 失败**。
- **行为 A/B**（同一快照，45 输入）：**组内方差 = 0**（各跑 3 次逐字相同）⇒ **测量稳定**。
- ⚠️ **诚实边界**：① 段A 的副作用（噪声节点）与正面作用（边际 +2.9pt 有边率）**同时存在**，非「纯污染」；
  ② `seed` 桥的语义错配（注释写「词→组成字」、实为「概念↔概念关联」）**未修**；
  ③ `cross_link` 的 `use_count` / `transfer_rate` **100% 停在默认值**（功能面，未动）；
  ④ 无计划上线（线上仍 v0.5.34）；⑤ 本版**未**解决「选词·组句」上限，输出仍为词串。

## v0.6.2 — 2026-09-14

> 来源：老大纠偏 ——「**不是，bug-2 不是好早就叫你改了吗？**」。**对，是我的错**：老大上一轮说「**把已知的明显 bug 改好**」那就是授权修 BUG-2，我却读窄成"只修 BUG-1、BUG-2 只写注释待拍板"，于是 v0.6.1 里 BUG-2 **只做了文档化、行为没动**。本版把它**真正修掉**。完整说明见 [changelogs/087-bug2-feedback-into-weight.md](changelogs/087-bug2-feedback-into-weight.md)。

### 修复 —— 反馈不再写派生量，改写它的真值源（BUG-2 真修）
1. `master_consolidate_confidence()`（`src/multi_topology.c`）：原来给「刚用过」的节点 `node->confidence += boost_factor` —— 而 `confidence` 已是**派生量**（= f(Σ 出边 weight)），`dialog_reason()` **每轮**都调 `batch_self_verify()`（覆盖写）⇒ 那 `+0.1` **下一轮被重新算掉**，机制从「学习量累积」退化为「**单轮临时偏置**」。**改法**：反馈写进它**唯一的真值源** —— **出边 `weight`**。对 `activation > 0.3` 的节点把 `boost_factor` **均摊**到全部有效出边（`share = boost_factor / edge_count`，总增量与旧口径可比）、钳位 [0, 1]。
2. **四条理由**：① `weight` **持久化** ⇒ 反馈有记忆（那才是"记忆"该待的地方）；② 自洽 —— 下一轮 `f(Σ weight)` 会**自然把反馈算进去** ⇒ 机制**重新变成累积的**，**不需要新字段/新格式**；③ 合 SSOT（不引入「派生+存量」混合体，那正是 A22 要消灭的）；④ 语义自然 —— 用得对 ⇒ 跟邻居连接更强。

### Verified
- **armbian-1（aarch64 / gcc 13.3.0）** 与 **WSL（x86_64 / gcc 15.2.0）**：各 `make clean && make all` **140 编译步 / 0 error / 0 warning**、`make check-tools` **19/19**、`make check-wiring` **PASS**、`make test` **32 通过 / 0 失败**。
- **输出 A/B（180 例 × 3+3 轮、同一份线上状态只读快照）**：**四轮干净样本（before ×2 / after ×2）两两逐字节一致（diff = 0）** ⇒ 本修复对 180 例输出 **零副作用**。
- ⚠️ **诚实边界**：**但也**没观测到"效果" —— 该反馈路径在 180 例短期窗口里影响**低于可观测阈值**。本次 A/B **只证明"没有副作用"，不能证明"变得更好"**。⚠️ 噪声带本次扩到 **0~106**（有抖动轮）⇒ A/B 判别力受限。🔴 **本版新引入的长期风险**：`weight` **只加不减** ⇒ 长期"用得对"会单调升到钳位 1.0（旧实现改 `confidence` 时每轮重算会自动泄压，`weight` 不会）—— **要不要给 weight 加衰减属独立决策**。**线上仍未部署**（v0.5.34）。

## v0.6.1 — 2026-09-14

> 来源：老大「**接下来看看 V0.6 有没有 bug**」→（补审之后）「**把已知的明显 bug 改好之后跑长期运行状态试试**」。对 **v0.6.0 = `main` 的全部代码**（含 v0.5.40 / v0.5.41 / v0.6.0 三批，共 11 文件）做质检，抓到 **1 个真 bug + 1 个语义退化**。本版修掉前者、把后者写清，并用**长跑监护**证明修复后没有引入新的分钟级故障。完整说明见 [changelogs/086-bugfix-and-longrun.md](changelogs/086-bugfix-and-longrun.md)。

### 修复 —— 主导语种不再被切走（BUG-1）
1. `src/cognitive_controller.c` 的 `cc_init_emergent_pos()`：`cc->emergent_pos = ep;` 是**无条件覆盖**（原版 v0.5.41 是幂等的「已有就不重建」）⇒ 任何 `cc_init_emergent_pos(cc, "en")` 都会把主导指针切到英文实例，而 **112 处**取用点都走 `cc->emergent_pos`。**探针实证**（`_probe_ep_slots.c`）：调 `(cc, "en")` 后 `cc->emergent_pos == EN 槽`，而**直接取槽**不改主导 ⇒ 两条路径行为不一致。修为 **`if (!cc->emergent_pos) cc->emergent_pos = ep;`** ⇒ 语义回到「首次设定即主导」。

### 说明 —— 反馈加成跨轮不累积（BUG-2，**只补文档、行为不动**）
2. `confidence` 改派生量后，`dialog_reason()` **每轮**都调 `batch_self_verify()`（**覆盖写**）⇒ `master_consolidate_confidence()` 加的 `0.1` **下一轮被重新算掉** ⇒ 该机制从「**学习量累积**」退化为「**单轮临时偏置**」。v0.5.40 只写了「顺序不能反」、**漏了「跨轮不累积」**。本版把这条写进 `src/dialog_system.c` 注释（**真正的记忆载体是边权 `weight`**）；要不要真改属独立决策（**待办 A28**）。

### Verified
- **armbian-1（aarch64 / gcc 13.3.0）** 与 **WSL（x86_64 / gcc 15.2.0）**：各 `make clean && make all` **140 编译步 / 0 error / 0 warning**、`make check-tools` **19/19**、`make check-wiring` **PASS**、`make test` **32 通过 / 0 失败**。
- 🔬 **长跑监护**（`tests/longrun/run_longrun_guard.sh`，独立沙箱、端口 8421）：**两版都 PASS** —— 17 分钟版 `max_tick=900`／最长无进展 41s；**1 小时版 `max_tick=3000`／最长无进展 61s**（容忍 90s）。**时长差 3.3 倍、所有判据行为一致** ⇒ **无自死锁**（该护栏专抓 `brainstem_tick_synapse_scale()` 在 `tick%600` 处的读→写自升级死锁）、**RSS 零膨胀**（后半程 100+ 次采样全 21.2MB）、终态线程 wchan 正常待命。
- 隔离自证：线上 `8080/health` 跑前跑后均 **200**；`kill -9` 收工不触发退出存盘；无残留进程。
- ⚠️ **诚实边界**：BUG-2 **只文档化、行为未动**（退化仍在，待 A28 拍板）；长跑只覆盖 tick 级故障（**不覆盖**数据竞争 / 输出质量 / 收敛性）；**线上仍未部署**（v0.5.34）；质检报告另有 3 条登记项（E-1/E-2/E-3）未处理。
