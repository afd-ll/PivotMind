...
int main(int argc, char* argv[]) {
    ...
    // 3. 加载特征向量 — 优先从 features.bin 读取训练后的特征
    printf("[3/5] 初始化特征向量...\n");
    int feat_loaded = load_features(master, "features.bin");
    if (feat_loaded > 0) {
        printf("     ✓ 从 features.bin 加载特征 (%d 节点)\n", feat_loaded);
    } else {
        printf("     - 未找到 features.bin, 跳过\n");
    }
    // 对剩余零特征节点（新节点）做随机初始化
    int initted = init_random_features(master);
    if (initted > 0) {
        printf("     ✓ 随机初始化特征向量 (%d 节点)\n", initted);
    } else {
        printf("     - 所有节点已有有效特征向量\n");
    }

    // 4. 加载/重建跨拓扑连接 — 优先加载已有文件
    printf("[4/5] 加载跨拓扑连接...\n");
    int cross_loaded = load_cross_edges(master, "cross_edges.bin");
    if (cross_loaded > 0) {
        printf("     ✓ 已加载跨拓扑连接 (%d 条)\n", cross_loaded);
    } else {
        printf("     - 未找到跨连接文件, 回退重建...\n");
    ...
    // 保存特征向量 + 跨连接
    // save_features(master, "features.bin");  // 不覆盖训练好的 features.bin

    // 注意: 不自动保存状态文件，避免覆盖训练数据
    printf("     ✓ 状态文件未修改\n");
