...
> 发版收尾时又补了一轮（**第七节**）：补跑 `make test` 挖出 Makefile / 脚本 / 测试里
> 同类的框病 —— 第一轮的取样口径只覆盖了 19 个工具二进制，漏了 `make test/test-fast`。
...
迁移后，**生产代码里手写制表框清零**。
原本残留 6 个只在 `tests/` 下（测试脚手架）—— **第二轮已连同 Makefile / 脚本一并收掉**（见第七节）。
...
## 七、第二轮：`make test` 的框 + 非 C 介质里的同类病症

### 7.1 缘起：跑 `make test` 才发现框是歪的

第一轮验证只跑了 19 个**工具二进制**的启动框矩阵，**没跑 `make test`**。
补跑时显示宽度检查器立刻报：

```
✗ make_test.log:70  宽度不一致 [40, 39, 40]
     w= 40 | ╔══════════════════════════════════════╗
     w= 39 | ║  结果: 27 通过,  0 失败             ║
     w= 40 | ╚══════════════════════════════════════╝
```

这三行来自 `Makefile:473` 与 `:499`（`test:` / `test-fast:` 各一份），
**是运行期可见的输出**，不属于「`tests/` 下的脚手架」——
第一轮把它划在范围之外，是取样口径的漏，不是判断错误。

### 7.2 一次性扫全仓（静态口径）

写了静态测量器把全仓**框字面量**抠出来逐块比宽度。格式符按「值不超宽」折算：
`%2d`→2 列、`%-16s`→16 列、**无宽度的 `%s` → 不可静态判定**（不计入结论）。
19 个框块里 13 块错位，修掉 11 处：

| 文件 | 位置 | 病 | 处理 |
|---|---|---|---|
| `Makefile` | 473 / 499 | 39 vs 40（运行期已确认） | 补 1 格 |
| `scripts/demo.sh` | 9-10 | 60 vs 65 | 补 5 格 |
| `tools/textbook_download_guide.py` | 30 | 65 vs 64 | 收 1 格 |
| `tests/test_runner.c` | 95 | `%d/%d` 不定宽 | 定宽为 `%2d/%2d` + 补 1 格 |
| `tests/test_cognitive_controller.c` | 234 | 55 vs 56 | 补 1 格 |
| `tests/test_cognitive_full.c` | 187 | 54 vs 56 | 补 2 格 |
| `tests/unit/test_web_fetch.c` | 619 | 53 vs 52 | 收 1 格 |

### 7.3 唯一一处真正治根的：`test_web_fetch` 汇总框

687-692 的汇总框是**两段式行**（行首 `║` 在一条 `printf`、行尾 `║` 在另一条，
中间夹 `if (跳过)` / `if (失败)` 两个分支），原来把尾部空格写死 12 格。
而本机实测 `tests_skipped = 9` —— **「(9 跳过)」是常态路径，不是罕见分支**：

```
✗ 61 vs 52   ← ║  测试汇总: 46/46 通过 (9 跳过)                            ║
```

写死空格必然被推走。改为：先在缓冲区拼好整行，再用 **`ui_disp_width()`** 算填充格数。
`ui.o` 本来就在 `libpivotmind.a` 里（测试链的是 `-lpivotmind`），所以**直接复用宽度真值源**，
不再另写一份宽度函数：

```c
char row[192];
size_t n = (size_t)snprintf(row, sizeof row, "  测试汇总: %d/%d 通过", tests_passed, tests_run);
if (tests_skipped > 0) n += (size_t)snprintf(row + n, sizeof row - n, " (%d 跳过)", tests_skipped);
if (tests_failed > 0)  n += (size_t)snprintf(row + n, sizeof row - n, " (%d 失败)", tests_failed);
int pad = 50 - ui_disp_width(row);          /* 52 宽边框 ⇒ 内宽 50 */
printf("║%s%*s║\n", row, pad > 0 ? pad : 0, "");
```

修后实测（WSL 与 armbian 逐字一致）：

```
║  测试汇总: 46/46 通过 (9 跳过)                   ║   ← 52 列，与边框齐
```

### 7.4 刻意没碰的（在此记账，不是遗漏）

- **`tools/run_parallel_train.sh`**（两个框，内容含 `$DATA_FILE` / `$EPOCHS` / `${total_min}`）：
  它是**死脚本** —— 要找的 `batch_learn_new[.exe]` 全仓已不存在（只剩 `batch_learn.c`），
  且 072 已记过它依赖的 `merge_states.py` 有独立缺陷。
  **打磨死代码是浪费**：正确动作是决定「删」还是「重写」，留给下一轮，本版不夹带。
- **`tests/scratch/*.c`**（2 个框）：不被 `Makefile` 引用 ⇒ 不参与构建，属沙盒遗留，同上。

### 7.5 第二轮验证

| 项 | WSL（x86_64 / gcc 15.2.0） | armbian（aarch64 / gcc 13.3.0） |
|---|---|---|
| `make all` | 0 error / 0 warning | 0 error / 0 warning |
| `make check-tools` | ✓ 19/19 | ✓ 19/19 |
| `make check-version` | PASS | PASS |
| `make test` | 27 通过 / 0 失败 | 27 通过 / 0 失败 |
| `make test-fast` | 16 通过 / 0 失败 | 16 通过 / 0 失败 |
| 直跑测试二进制 | 6/6 RC=0 | 6/6 RC=0 |
| **显示宽度检查器** | **9 框块 / 0 错位** | **23 框块 / 0 错位** |

armbian 的 23 块里含**重跑一遍的工具矩阵**（沙箱数据根），确认第一轮的框修复无回归；
`state_dump` 仍为 `子拓扑数 12 / 总节点数 530 / 总连接数 480`。
线上实例 `pid 1948532` 全程未动（`ELAPSED 1-02:33:38`，仅监听 `127.0.0.1:8080`）。

### 7.6 自己踩的坑（记下来，别复现）

`_p5_webfetch.py` 第一版**先插 `#include "ui.h"`、再按此前算出的行号替换框块** ——
插入让行号整体下移 1，`top/bot` 成了陈旧索引 ⇒ **底边框被重复了一遍**。
`git checkout` 回滚后改成「先替换、后插入」。
教训：**行号索引不能跨变异复用**，要么先改后定位，要么重新定位。
...
6. **手写框的迁移范围**（第二轮后更新）：`Makefile` / `scripts/demo.sh` /
   `tools/textbook_download_guide.py` / 4 个 `tests/*.c` 已全部对齐，
   全仓运行期可见的框块错位为 **0**。
   仍**刻意不碰**的只剩两处**死代码**：`tools/run_parallel_train.sh`
   （依赖已不存在的 `batch_learn_new[.exe]`）与 `tests/scratch/*.c`
   （不被 `Makefile` 构建）—— 它们该被删除或重写，而不是继续修框（详见 7.4）。
