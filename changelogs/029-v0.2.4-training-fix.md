...
// 修复后：Q→A 直接强连接
int q_id = huarong_net_find_concept(vocab->net, stream->q_buf);
if (q_id < 0) q_id = huarong_net_dynamic_add_node(vocab->net, stream->q_buf, NULL, 0);
int a_id = huarong_net_find_concept(vocab->net, stream->a_buf);
if (a_id < 0) a_id = huarong_net_dynamic_add_node(vocab->net, stream->a_buf, NULL, 0);
if (q_id >= 0 && a_id >= 0) {
    huarong_net_add_connection(vocab->net, q_id, a_id, 0.8f);
...
**问题**：`huarong_net_add_connection` 内部有去重机制（`node_conn_find` O(1) 哈希查找），重复连接只更新权重不创建新边。但外部计数 `total_added_edges++` 在每次调用时都递增，导致统计虚高。
