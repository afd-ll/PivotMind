...
**文件**：`include/huarong_topology.h`，`src/huarong_topology.c`

`HuarongTopologyNet` 新增 `concept_hash`（开放定址哈希表）：
...
```c
// O(1) 按名称查找概念
ReasoningNode* huarong_net_find_concept(HuarongTopologyNet* net, const char* concept);

// 内部：插入哈希表
static int _concept_hash_insert(HuarongTopologyNet* net, ReasoningNode* node);
```
...
```c
// 突触缩放核心逻辑
void synaptic_scaling(HuarongTopologyNet* net) {
...
- **堆损坏未修复**：`huarong_net_dynamic_add_node` 内部无去重，12 万次调用后堆碎片导致崩溃（v0.2.3 修复）
