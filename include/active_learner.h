...
typedef struct {
    ...
    // 上一轮路径信息（供 feedback_correct 定位需压制的边）
    int last_path_node_ids[PM_PATH_TRACK];     // 路径节点ID序列
    int last_path_topo_types[PM_PATH_TRACK];   // 路径节点所属拓扑类型
    int last_path_edge_ids[PM_PATH_TRACK];     // 路径上使用的边索引
    int last_path_count;             // 路径长度
} ActiveLearner;
