...
static void add_or_strengthen(HuarongTopologyNet* net, ReasoningNode* from, ReasoningNode* to,
    ...
    // 新建边 — 初始置信度0.5, 偏差0.5 (由 huarong_net_add_connection 处理)
...
static int process_qa_data(MasterTopology* master, SubTopology* vocab_sub,
    ...
    for (int ep = 0; ep < epochs; ep++) {
        int upd = 0;
        #pragma omp parallel for reduction(+:upd) schedule(dynamic, 100)
...
int main(int argc, char* argv[]) {
    ...
    int qa_edges = 0;
    if (access(qa_file, F_OK) == 0) {
        qa_edges = process_qa_data(master, vocab_sub, qa_file, epochs);
    } else {
        printf("  ⚠ QA 文件不存在: %s\n", qa_file);
    }
