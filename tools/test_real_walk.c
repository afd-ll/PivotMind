/**
 * 真实数据走边测试
 * 加载训练好的模型，用 topology_walk_greedy 测具体路�? */
...
// 全局状态文件路�?#define STATE_FILE "pivotmind_state.dat"
...
static void test_real_walk(MasterTopology* master, const char* start_concept, int topo_type) {
    ...
    if (start->connection_count == 0) {
        ...
        for (int i = 0; i < start->connection_count && printed < 5; i++) {
            ...
            score *= vmod;
            printf("    �?%s (权重=%.3f conf=%.3f 目标激�?%.3f 综合=%.4f)\n",
                   target->concept,
    ...
    int path[32];
    float scores[32];
    int len = topology_walk_greedy(sub, start->node_id, path, scores, 20, NULL, 1.0f, NULL, NULL);

    printf("  [走边] 从�?s」出�?", start_concept);
    for (int i = 0; i < len; i++) {
        int nid = path[i];
...
int main() {
    ...
    // 加载状�?    MasterTopology* master = master_topology_create(10);
    ...
    master_add_sub_topology(master, TOPO_CONTEXT, "上下文拓�?, 1000, 5);
    ...
    // ===== 测试核心序列 =====
    printf("--- 测试: 人工智能是什�?---\n");
    test_real_walk(master, "�?, TOPO_VOCABULARY);
    test_real_walk(master, "�?, TOPO_VOCABULARY);
    test_real_walk(master, "�?, TOPO_VOCABULARY);
    test_real_walk(master, "�?, TOPO_VOCABULARY);
    test_real_walk(master, "�?, TOPO_VOCABULARY);
    ...
    printf("\n--- 多概念起点测试（语义拓扑�?--\n");
    // 模拟联想引擎的起始方式：从最高激活节点开�?    SubTopology* vocab = master_get_sub_topology_by_type(master, TOPO_VOCABULARY);
    if (vocab && vocab->net) {
        // 找激活最高的5个节�?        printf("词汇拓扑中激活最高的节点:\n");
        int top_n = 0;
        ...
        if (top_n == 0) printf("  (无高激活节�?\n");
    }

    printf("\n--- 激活所有测试概念后再走�?---\n");
    // 模拟推理后的激活状态：激�?�?"�?"�?"�?"�?"什""�?
    const char* test_concepts[] = {"�?,"�?,"�?,"�?,"�?,"什","�?,"�?,"�?,"�?,"�?};
