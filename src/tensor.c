...
// 元素级运�?
// Broadcast index helper: map flat output index to source tensor flat index,
// handling dims of size 1 (broadcast) and fewer dims (prepend dims of size 1).
static size_t broadcast_index(size_t out_idx, Tensor* t, size_t out_ndim, const size_t* out_shape) {
    size_t src_idx = 0;
    size_t remaining = out_idx;
    
    for (int d = (int)out_ndim - 1; d >= 0; d--) {
        size_t dim_size = out_shape[d];
        size_t coord = remaining % dim_size;
        remaining /= dim_size;
        
        int t_dim = d - (int)(out_ndim - t->ndim);
        if (t_dim < 0) continue;       // t has fewer dims => broadcast
        
        if (t->shape[t_dim] == 1) continue;  // size-1 dim => broadcast
        
        src_idx += coord * t->strides[t_dim];
    }
...
Tensor* tensor_add(Tensor* a, Tensor* b)
{
    ...
    if (a->dtype != DT_FLOAT32 && a->dtype != DT_FLOAT64) return NULL;
    ...
    float* a_f = (float*)a->data;
    float* b_f = (float*)b->data;
    float* r_f = (float*)result->data;
    
    for (size_t i = 0; i < out_size; i++) {
        size_t a_idx = broadcast_index(i, a, out_ndim, out_shape);
        size_t b_idx = broadcast_index(i, b, out_ndim, out_shape);
        r_f[i] = a_f[a_idx] + b_f[b_idx];
    }
...
Tensor* tensor_div(Tensor* a, Tensor* b)
{
    ...
    for (size_t i = 0; i < out_size; i++) {
        size_t a_idx = broadcast_index(i, a, out_ndim, out_shape);
        size_t b_idx = broadcast_index(i, b, out_ndim, out_shape);
        r_f[i] = b_f[b_idx] != 0.0f ? a_f[a_idx] / b_f[b_idx] : 0.0f;
    }
...
{
    if (!tensor || !shape || ndim == 0) return NULL;
    // 检查广播兼容性
    for (size_t i = 0; i < ndim; i++) {
        size_t t_dim = (i < tensor->ndim) ? tensor->shape[tensor->ndim - 1 - i] : 1;
        size_t s_dim = shape[ndim - 1 - i];
        if (t_dim != s_dim && t_dim != 1) return NULL;
    }

    Tensor* out = tensor_create(tensor->dtype, ndim, shape);
    if (!out) return NULL;

    size_t elem_size = tensor_element_size(tensor->dtype);
    unsigned char* src = (unsigned char*)tensor->data;
    unsigned char* dst = (unsigned char*)out->data;

    // 预计算输出和源张量的 stride
    size_t out_strides[16] = {0};
    size_t src_strides[16] = {0};
    out_strides[ndim - 1] = 1;
    for (int d = (int)ndim - 2; d >= 0; d--) {
        out_strides[d] = out_strides[d + 1] * shape[d + 1];
    }

    size_t src_offset = ndim - tensor->ndim;
    if (tensor->ndim > 0) {
        src_strides[tensor->ndim - 1] = 1;
        for (int d = (int)tensor->ndim - 2; d >= 0; d--) {
            src_strides[d] = src_strides[d + 1] * tensor->shape[d + 1];
        }
    }

    // 逐元素复制，通过 stride 查表计算源索引
    for (size_t i = 0; i < out->size; i++) {
        ...
        for (int d = (int)ndim - 1; d >= 0; d--) {
            ...
            remaining /= shape[d];
            int t_d = d - (int)src_offset;
            if (t_d >= 0 && t_d < (int)tensor->ndim) {
                if (tensor->shape[t_d] == 1) dim_idx = 0;
                src_idx += dim_idx * src_strides[t_d];
            }
        }

        memcpy(dst + i * elem_size, src + src_idx * elem_size, elem_size);
    }
...
void tensor_enable_pool(bool enable) {
    if (enable && !global_pool) {
        ...
        LOG_INFO("Tensor pool stats: allocated=%zu, freed=%zu",
                  atomic_load(&total_mallocs), atomic_load(&total_frees));
...
void tensor_get_pool_stats(size_t* allocated, size_t* freed) {
    if (allocated) *allocated = atomic_load(&total_mallocs);
    if (freed) *freed = atomic_load(&total_frees);
}
