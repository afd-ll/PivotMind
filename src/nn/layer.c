...
void layer_linear_backward(Layer* layer, Tensor* grad_output) {
    ...
    // 保存输入用于梯度计算(需要在前向传播时保�?
    // 这里简化处理，假设layer->private_data保存了输�?
    ...
    for (size_t o = 0; o < output_size; o++) {
        ...
        grad_b_data[o] += sum;  // 累加而不是覆�?
    }

    // 注意:完整的权重梯度需要输入数�?
    // grad_weights = input^T @ grad_output
    // 由于没有保存输入，这里返回NULL调用者可以自行计�?
    // 在seq2seq_train.c中已经通过矩阵乘法计算�?
...
/* Glorot initialization (alias for Xavier) */
