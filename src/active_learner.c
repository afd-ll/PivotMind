...
void discover_on_snapshot(TopoSnapshot* snap, int window_size, int verbose);
...
ActiveLearner* active_learner_create(MasterTopology* master, MemorySystem* memory) {
    ...
    learner->learning_interval = PM_ACTIVE_LEARNER_INTERVAL;  // 默认5分钟一次
...
void active_learner_destroy(ActiveLearner* learner) {
    ...
    int was_verbose = learner->verbose;
    ...
    if (was_verbose) printf("[主动学习] 学习器已销毁\n");
}
...
void* learning_cycle(void* arg) {
    ...
    while (learner->is_running) {
        ...
        if (snap_vocab) {
            ...
            discover_on_snapshot(snap_vocab, 2, learner->verbose);
            if (learner->verbose) printf("  扫描完成...\n");
        }
        for (int i = 0; i < snap_count; i++) {
            discover_on_snapshot(snapshots[i], 2, learner->verbose);
        }
        ...
        if (total_merged > 0) {
            ...
            if (learner->verbose) printf("  ✓ 合并 %d 条新关系\n", total_merged);
        }
        ...
        // 打印统计
        if (learner->verbose) print_learning_stats(learner);
        
        // 等待下一个周期
        if (learner->verbose) printf("[主动学习] 下一周期等待 %d 秒...\n", learner->learning_interval);
...
void active_learner_start(ActiveLearner* learner) {
    ...
    if (learner->verbose) printf("[主动学习] 学习器已启动，PID: %lu\n", (unsigned long)learner->thread);
}
...
void active_learner_stop(ActiveLearner* learner) {
    ...
    if (learner->verbose) printf("[主动学习] 学习器已停止\n");
}
...
void active_learner_set_verbose(ActiveLearner* learner, int verbose) {
    if (!learner) return;
...
void learn_from_memory(ActiveLearner* learner) {
    ...
    if (results && result_count > 0) {
        if (learner->verbose) printf("  待巩固条目: %d\n", result_count);
    ...
    if (learned_count > 0) {
        ...
        if (learner->verbose) printf("  ✓ 从记忆提取了 %d 个新概念\n", learned_count);
    } else {
        if (learner->verbose) printf("  - 暂无新知识需要学习\n");
    }
}
...
// ==================== P2: 自动连接发现与特征向量 ====================
// cosine_similarity 使用 common.h 中的 static inline 版本
...
void learn_from_dialog(ActiveLearner* learner, const char* user_input,
    ...
    // 精确路径节点更新（替代全拓扑 strstr 扫描）
    if (learner->last_path_count > 0) {
        for (int pi = 0; pi < learner->last_path_count && pi < PM_PATH_TRACK; pi++) {
            ...
            if (topo_id < 0 || topo_id >= learner->master->sub_topo_count) continue;
            SubTopology* sub = learner->master->sub_topologies[topo_id];
            if (!sub || !sub->net || node_id < 0 || node_id >= sub->net->node_count) continue;
    ...
    if (!is_correct && learner->last_path_count > 0) {
        ...
        for (int pi = 0; pi < learner->last_path_count && pi < PM_PATH_TRACK; pi++) {
            int topo_id = learner->last_path_topo_types[pi];
            int edge_idx = learner->last_path_edge_ids[pi];
            if (topo_id < 0 || topo_id >= learner->master->sub_topo_count || edge_idx < 0)
                continue;
            SubTopology* sub = learner->master->sub_topologies[topo_id];
            if (!sub || !sub->net) continue;
            int node_id = learner->last_path_node_ids[pi];
