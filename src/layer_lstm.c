...
void lstm_backward_step(LSTMLayer* layer, Tensor* dh_next, Tensor* dc_next) {
    ...
    // Store for BPTT chain
    if (layer->dh_prev_out) tensor_destroy(layer->dh_prev_out);
    layer->dh_prev_out = dh_prev_final;
    if (layer->dc_prev_out) tensor_destroy(layer->dc_prev_out);
    layer->dc_prev_out = dc_prev;
}

// LSTM反向传播(序列)
void lstm_backward_sequence(LSTMLayer* layer, Tensor* x_seq, Tensor* dh_seq) {
    if (!layer || !x_seq || !dh_seq) return;
    if (x_seq->ndim < 2 || dh_seq->ndim < 2) {
        LOG_ERROR("LSTM sequence BP: x_seq and dh_seq must be 2D");
        return;
    }
    if (x_seq->shape[0] != dh_seq->shape[0]) {
        LOG_ERROR("LSTM sequence BP: seq_len mismatch");
        return;
    }
    ...
    Tensor* dh_next = NULL;
    Tensor* dc_next = NULL;
    Tensor* saved_h_prev = NULL;  // h_t from time step t+1 -> h_prev for step t
    Tensor* saved_c_prev = NULL;  // c_t from time step t+1 -> c_prev for step t
    ...
    for (int t = (int)seq_len - 1; t >= 0; t--) {
        ...
        // Re-run forward step with temporal chain
        Tensor* h_out = lstm_forward_step(layer, x_t_sq, saved_h_prev, saved_c_prev);

        // Combine with BPTT gradients
        Tensor* dh_c;
        if (dh_next) {
            dh_c = tensor_add(dh_t_sq, dh_next);
            tensor_destroy(dh_next);
        } else {
            dh_c = tensor_clone(dh_t_sq);
        }
        dh_next = NULL;

        Tensor* dc_c = NULL;
        if (dc_next) {
            dc_c = tensor_clone(dc_next);
            tensor_destroy(dc_next);
        }
        dc_next = NULL;

        // Backward step (stores dh_prev_out/dc_prev_out in layer)
        lstm_backward_step(layer, dh_c, dc_c);

        // Retrieve BPTT chain gradients
        if (layer->dh_prev_out) {
            dh_next = tensor_clone(layer->dh_prev_out);
        }
        if (layer->dc_prev_out) {
            dc_next = tensor_clone(layer->dc_prev_out);
        }

        // Update saved state for previous time step
        if (saved_h_prev) tensor_destroy(saved_h_prev);
        if (saved_c_prev) tensor_destroy(saved_c_prev);
        saved_h_prev = layer->h_prev ? tensor_clone(layer->h_prev) : NULL;
        saved_c_prev = layer->c_prev ? tensor_clone(layer->c_prev) : NULL;
...
void lstm_layer_destroy(LSTMLayer* layer) {
    ...
    tensor_destroy(layer->dc_prev_out);
