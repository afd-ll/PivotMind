...
bool is_visited(AssociativeEngine* engine, int topo_id, int node_id) {
    int key = topo_id * MAX_NODES_PER_TOPO + node_id;
...
void propagate_association(AssociativeEngine* engine,
    ...
    // 递归深度硬上限（防止visited满时循环→栈溢出）
    engine->recursion_depth++;
    if (engine->recursion_depth > MAX_RECURSION_DEPTH) {
        engine->recursion_depth--;
        return;
    }
...
static int assoc_cmp_desc(const void* a, const void* b) {
    float diff = ((const Association*)b)->activation - ((const Association*)a)->activation;
...
char* generate_from_associations(AssociativeEngine* engine, int max_len,
    ...
    qsort(engine->associations, engine->assoc_count, sizeof(Association), assoc_cmp_desc);
    ...
    for (int start_i = 0; start_i < engine->assoc_count && start_i < 5 && pos < max_len - 10; start_i++) {
        ...
        if (has_cross_links) {
            ...
            for (int p = 1; p < path_len && pos < max_len - 10; p++) {
                ...
                // 追加到 avoid_chars（跨路径防回声）
                for (const char* cp = node->concept; *cp && ac < 250; ) {
                    int cl = 1;
                    if ((unsigned char)*cp >= 0xC0 && (unsigned char)*cp < 0xE0) cl = 2;
                    else if ((unsigned char)*cp >= 0xE0 && (unsigned char)*cp < 0xF0) cl = 3;
                    else if ((unsigned char)*cp >= 0xF0) cl = 4;
                    for (int b = 0; b < cl && ac < 250; b++)
                        avoid_chars[ac++] = *cp++;
                }
                avoid_chars[ac] = '\0';
            }
