...
static float semantic_consistency(GeneratedSequence* seq,
    ...
    /* 在语义拓扑中找 input 的激活节点 */
    SubTopology* sem = NULL;
    for (int t = 0; t < topo->sub_topo_count; t++) {
        SubTopology* sub = topo->sub_topologies[t];
        if (sub && sub->type == TOPO_SEMANTIC) { sem = sub; break; }
    }
    ...
    for (int i = 0; i < seq->count && i < MAX_GENERATED_WORDS; i++) {
        ...
        if (i < seq->count - 1) {
            ...
            if (na && nb) {
                ...
                for (int k = 0; k < na->connection_count; k++) {
                    if (na->connections[k] == nb) {
                        float edge_w = na->connection_weights[k];
...
static float template_fit(GeneratedSequence* seq, MasterTopology* topo) {
    ...
    for (int t = 0; t < topo->sub_topo_count; t++) {
        SubTopology* sub = topo->sub_topologies[t];
        if (sub && sub->type == TOPO_TEMPLATE) { tpl = sub; break; }
    }
    ...
    for (int i = 0; i < 50 && i < tpl->net->node_count; i++) {
        ...
        if (n) { total_conns += n->connection_count; sampled++; }
    }
...
static float emotion_consistency(GeneratedSequence* seq, MasterTopology* topo) {
    ...
    for (int t = 0; t < topo->sub_topo_count; t++) {
        SubTopology* sub = topo->sub_topologies[t];
        if (sub && sub->type == TOPO_EMOTION) { emo = sub; break; }
    }
